...
객체의 복사는 객체 생성의 또 다른 방법으로 new 연산자를 통한 객체 생성 이외의 다른 방법을 제공한다.<br/>
클로닝을 통한 객체 복사를 이용해 객체를 생성할 경우 객체의 생성자 메소드를 실행하지 않으며,<br/>
객체가 유지하고 있는 상태값들을 그대로 복사하게 된다.<br/>
이때 복사되는 원래 객체에는 아무런 변화를 주지 않는다.<br/>
